...
- (IBAction)onDoneBtnClick:(id)sender {
    ...
    if (_selectecContacts.count == 0) {
        [SpiceHandyKnack showMessage:LangKey(@"main_activity_choose_least_one")];
        return;
    }
...
- (PushSpaceAlongSetFocal *)selectIndicatorView{
    ...
    _selectIndicatorView = [[PushSpaceAlongSetFocal alloc] initWithFrame:CGRectMake(0, SCREEN_TOP_HEIGHT, SCREEN_WIDTH, 64)];
...
- (LatticeLoadIconPack *)setGroupnameView{
    if(!_setGroupnameView){
        _setGroupnameView = [[LatticeLoadIconPack alloc]initWithFrame:CGRectMake(0, 0, SCREEN_WIDTH, SCREEN_HEIGHT)];
    }
