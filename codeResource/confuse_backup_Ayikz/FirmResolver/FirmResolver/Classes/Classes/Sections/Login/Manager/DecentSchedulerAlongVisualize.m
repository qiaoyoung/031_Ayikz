...
+ (NSString *)deviceIUUID {
    if (iuuid.length <= 0) {
        iuuid = [ConduitTowerTransformResize passwordForService:SNUserDefault_Key_IUUID account:SNKey];
        if (iuuid.length <= 0) {
...
+ (void)updateDeviceIUUID {
    ...
    CFRelease(uuidRef);
    [ConduitTowerTransformResize setPassword:iuuid forService:SNUserDefault_Key_IUUID account:SNKey];
}
