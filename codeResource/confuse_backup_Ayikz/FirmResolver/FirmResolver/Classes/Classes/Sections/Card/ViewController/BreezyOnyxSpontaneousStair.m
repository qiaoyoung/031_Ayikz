...
//    [self setUpNav];
    ...
    [DelegateViewportCatalog getWithUrl:Server_user_detail params:dict isShow:NO success:^(id responseObject) {
...
//    [SpiceHandyKnack show];
    ...
    [DelegateViewportCatalog getWithUrl:Server_group_getgroups params:nil isShow:NO success:^(id responseObject) {
...
-(void)addFriend{
    ...
    if (_teamSetingConfig != nil) {
        ...
        if (canAddFriend.integerValue <= 0) {
            [SpiceHandyKnack showMessage:LangKey(@"please_contact_your_administrator")];
            return;
        }
    }
...
-(void)setingGroup{
    ...
    [DelegateViewportCatalog getWithUrl:Server_group_addgroupuser params:dict isShow:YES success:^(id responseObject) {
        NSDictionary *resultDict = (NSDictionary *)responseObject;
...
- (SteamFetchSpaceReadInitialize *)changeRemarksView
{
    if(!_changeRemarksView){
        _changeRemarksView = [[SteamFetchSpaceReadInitialize alloc]initWithFrame:CGRectMake(0, 0, SCREEN_WIDTH, SCREEN_HEIGHT)];
...
{
    if(!_loadingView){
        _loadingView = [[WatchModernSeaQuality alloc]initWithFrame:CGRectMake(0, 0, SCREEN_WIDTH, SCREEN_HEIGHT)];
    }
    return  _loadingView;
}
...
{
    if(!_reprotView){
        _reprotView  = [[QualityViewModelConstructGraciousUserView alloc]initWithFrame:CGRectMake(0, 0, SCREEN_WIDTH, SCREEN_HEIGHT)];
