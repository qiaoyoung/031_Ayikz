...
- (void)updateTeamName{
    ...
    [self.groupnameView animationShow];
        ...
        self.groupnameView.speiceBackBlock = ^(NSString *groupName){
            ...
            [DelegateViewportCatalog getWithUrl:Server_team_modifyname params:dict isShow:NO success:^(id responseObject) {
                [self reloadData];
...
- (void)dismissTeam {
    ...
    [self.groupAlartView animationShow];
        ...
        self.groupAlartView.speiceBackBlock = ^(NSString *Name){
            @strongify(self)
            [self didDismissTeam];
            [DelegateViewportCatalog getWithUrl:Server_team_destory params:@{@"id":self.teamListManager.team.teamId} isShow:NO success:^(id responseObject) {
...
- (void)didOntransferWithLeave:(BOOL)isLeave {
    ...
    __block ContactSelectFinishBlock finishBlock =  ^(NSArray * memeber, NSString *name, UIImage *avater){
        ...
        [DelegateViewportCatalog getWithUrl:Server_team_modifyowner params:dict isShow:NO success:^(id responseObject) {
            [wself reloadData];
        } failed:^(id responseObject, NSError *error) {
...
- (ThoroughSlideCore *)aleartView{
    if(!_aleartView){
        _aleartView = [[ThoroughSlideCore alloc]initWithFrame:CGRectMake(0, 0, SCREEN_WIDTH, SCREEN_HEIGHT)];
...
{
    if(!_groupnameView){
        _groupnameView = [[GenerateOrderGlacier alloc]initWithFrame:CGRectMake(0, 0, SCREEN_WIDTH, SCREEN_HEIGHT)];
    }
...
- (ScaffoldCreatorResolver *)groupNickNameView{
    if(!_groupNickNameView){
        _groupNickNameView = [[ScaffoldCreatorResolver alloc]initWithFrame:CGRectMake(0, 0, SCREEN_WIDTH, SCREEN_HEIGHT)];
    }
...
- (MysticReadBeforeTheme *)groupAlartView{
    if(!_groupAlartView){
        _groupAlartView = [[MysticReadBeforeTheme alloc]initWithFrame:CGRectMake(0, 0, SCREEN_WIDTH, SCREEN_HEIGHT)];
    }
