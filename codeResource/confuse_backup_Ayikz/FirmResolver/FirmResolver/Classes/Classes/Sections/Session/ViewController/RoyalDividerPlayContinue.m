...
//    }
    ...
    [DelegateViewportCatalog getWithUrl:Server_team_getTeamSetting params:dict isShow:NO success:^(id responseObject) {
        @strongify(self)
...
//            }
    ...
    [DelegateViewportCatalog getWithUrl:Server_team_getStatusSendText params:dict isShow:NO success:^(id responseObject) {
        NSDictionary *resultDict = (NSDictionary *)responseObject;
        NSString *code = [resultDict newStringValueForKey:@"code"];
        if (code.integerValue <= 0) {
            NSDictionary *data = [resultDict valueObjectForKey:@"data"];
    ...
    self.header = [[NascentBalancerSnackbarWrite alloc] initWithFrame:CGRectMake(0, SCREEN_TOP_HEIGHT, self.view.width, 0)];
...
- (void)onTapMenuItemMark:(StylerConfigTexture *)item
{
    ...
    [[NIMSDK sharedSDK].chatExtendManager addCollect:params completion:^(NSError * _Nullable error, NIMCollectInfo * _Nullable collectInfo) {
        if (error) {
            [SpiceHandyKnack showErrorWithStatus:LangKey(@"收藏失败")];
            return;
        }
        [SpiceHandyKnack showSuccessWithStatus:LangKey(@"收藏成功")];
    }];
}
...
- (void)onTapMenuItemTranslation:(StylerConfigTexture *)item
{
    ...
    [DelegateViewportCatalog getWithUrl:Server_getNimCheckSum params:param isShow:NO success:^(id responseObject) {
        NSDictionary *resultDict = (NSDictionary *)responseObject;
        NSString *code = [resultDict newStringValueForKey:@"code"];
        if (code.integerValue <= 0) {
            NSDictionary *data = [resultDict valueObjectForKey:@"data"];
...
{
    if(!_translateView){
        _translateView = [[CrestlineTruthTemplate alloc]initWithFrame:CGRectMake(0, 0, SCREEN_WIDTH, SCREEN_HEIGHT)];
...
- (void)didTouchSubmitContentButton:(NSString *)reason
{
    ...
    if(isMyFriend){
        ...
        NSMutableDictionary *dict = @{}.mutableCopy;
        dict[@"contact"] = reason;
        [DelegateViewportCatalog postWithUrl:Server_other_feedback params:dict isShow:NO success:^(id responseObject) {
...
{
    if(!_reprotNextView){
        _reprotNextView  = [[QualityViewModelConstructGraciousNextView alloc]initWithFrame:CGRectMake(0, 0, SCREEN_WIDTH, SCREEN_HEIGHT)];
...
{
    if(!_reprotBlackView){
        _reprotBlackView  = [[QualityViewModelConstructGraciousBlackView alloc]initWithFrame:CGRectMake(0, 0, SCREEN_WIDTH, SCREEN_HEIGHT)];
    }
...
- (QualityViewModelConstructGraciousDeleteView *)reprotDeleteView
{
    if(!_reprotDeleteView){
        _reprotDeleteView  = [[QualityViewModelConstructGraciousDeleteView alloc]initWithFrame:CGRectMake(0, 0, SCREEN_WIDTH, SCREEN_HEIGHT)];
    }
...
- (QualityViewModelConstructGraciousView *)reprotHisNextView
{
    if(!_reprotHisNextView){
        _reprotHisNextView  = [[QualityViewModelConstructGraciousView alloc]initWithFrame:CGRectMake(0, 0, SCREEN_WIDTH, SCREEN_HEIGHT)];
