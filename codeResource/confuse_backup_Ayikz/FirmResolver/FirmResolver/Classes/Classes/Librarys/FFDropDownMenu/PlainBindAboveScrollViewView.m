    ...
    if (_tableView == nil) {
        ...
        if ([self.cellClassName hasSuffix:@".xib"]) { //xib名称
            ...
            if (![NSClassFromString(className) isSubclassOfClass:[PlainBindAboveScrollViewBasedCell class]]) {
                FFLog(@"%@这个类不是PlainBindAboveScrollViewBasedCell的子类,请继承这个类",className);
                return _tableView;
            }
            ...
            if (![NSClassFromString(self.cellClassName) isSubclassOfClass:[PlainBindAboveScrollViewBasedCell class]]) {
                FFLog(@"%@这个类不是PlainBindAboveScrollViewBasedCell的子类,请继承这个类",self.cellClassName);
                return _tableView;
            }
...
- (void)setMenuAnimateType:(PlainBindAboveScrollViewViewAnimateType)menuAnimateType { //15
