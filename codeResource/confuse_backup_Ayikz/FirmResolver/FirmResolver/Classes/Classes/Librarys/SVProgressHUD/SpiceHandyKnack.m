...
#error SpiceHandyKnack is ARC only. Either turn on ARC for the project or use -fobjc-arc flag
#endif
...
+ (SpiceHandyKnack*)sharedView {
    ...
    static SpiceHandyKnack *sharedView;
     ...
     dispatch_once(&once, ^{ sharedView = [[self alloc] initWithFrame:[SpiceHandyKnack mainWindow].bounds]; });
#else
...
- (void)updateHUDFrame {
    ...
    hudWidth = SpiceHandyKnackHorizontalSpacing + MAX(labelWidth, contentWidth) + SpiceHandyKnackHorizontalSpacing;
    ...
    CGFloat centerY;
    if(self.statusLabel.text) {
        CGFloat yOffset = MAX(SpiceHandyKnackVerticalSpacing, (self.minimumSize.height - contentHeight - SpiceHandyKnackLabelSpacing - labelHeight) / 2.0f);
...
#else
            // If SpiceHandyKnack is used inside an app extension add it to the given view
