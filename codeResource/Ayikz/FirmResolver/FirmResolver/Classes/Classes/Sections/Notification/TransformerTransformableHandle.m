        ...
        case NIMSystemNotificationTypeFriendAdd:
        {
            ...
            [[[NIMSDK sharedSDK] userManager] requestFriend:request
                                                 completion:^(NSError *error) {
                                                     if (!error) {
                                                         [wself.navigationController.view makeToast:LangKey(@"successful_authentication")//@"验证成功".user_localized
                                                                                           duration:2
                                                                                           position:CSToastPositionCenter];
                                                         notification.handleStatus = DatasetterResolverDeltaTypeOk;
...
//                                                         [[NIMSDK sharedSDK].chatManager sendMessage:message toSession:session error:&error];
                                                     ...
                                                     {
                                                         [wself.navigationController.view makeToast:LangKey(@"fail_authentication")//@"验证失败,请重试".user_localized
                                                                                           duration:2
                                                                                           position:CSToastPositionCenter];
                                                     }
                                                     [wself.tableView reloadData];
