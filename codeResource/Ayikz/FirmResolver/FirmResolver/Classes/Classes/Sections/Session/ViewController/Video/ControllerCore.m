...
@property (nonatomic, strong) SiteConverterStarLayout *videoSlider;  //滑动条
...
- (SiteConverterStarLayout *)videoSlider{
    if (!_videoSlider) {
        _videoSlider = [[SiteConverterStarLayout alloc] initWithFrame:CGRectMake(0, 0, 3*(SCREEN_WIDTH/4), 20)];
...
{
    if(!_loadingView){
        _loadingView = [[WatchModernSeaQuality alloc]initWithFrame:CGRectMake(0, SCREEN_TOP_HEIGHT, SCREEN_WIDTH, SCREEN_HEIGHT-SCREEN_TOP_HEIGHT-SCREEN_TABBAR_HEIGHT)];
    }
    return  _loadingView;
}
