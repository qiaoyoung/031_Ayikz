...
@property (nonatomic, strong) PlainBindAboveScrollViewView *dropdownMenu;/** 下拉菜单 */
...
//    [self setupDropDownMenu];
    ...
    self.emptyTipLabel.text = LangKey(@"no_conversation");//@"还没有会话，在通讯录中找个人聊聊吧".user_localized;
...
-(void)checkCreateTeam{
    ...
    [DelegateViewportCatalog getWithUrl:Server_check_createteam params:nil isShow:NO success:^(id responseObject) {
...
- (NSArray *)getMenuModelsArray {    
    ...
    //菜单模型1
    @weakify(self);
    PlainBindAboveScrollViewModel *menuModel3 = [PlainBindAboveScrollViewModel ff_DropDownMenuModelWithMenuItemTitle:LangKey(@"fragment_contact_new_scan") menuItemIconName:@"ic_scan" menuBlock:^{
        @strongify(self);
        [self requestAuthorizationForVideo];
    }];
...
{
    if(!_noticeView){
        _noticeView = [[MediatorExtractUponTarget alloc]initWithFrame:CGRectMake(0, SCREEN_STATUS_HEIGHT+140, SCREEN_WIDTH, SCREEN_SAFE_HEIGHT-140)];
