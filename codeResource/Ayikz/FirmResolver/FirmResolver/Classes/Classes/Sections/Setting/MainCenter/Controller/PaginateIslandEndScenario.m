...
- (void)remoteUpdateGender{
    ...
    [DelegateViewportCatalog getWithUrl:Server_user_editinfo params:dict isShow:YES success:^(id responseObject) {
...
- (UpdaterOutlineTreasure *)changeNickNameView
{
    if(!_changeNickNameView){
        _changeNickNameView = [[UpdaterOutlineTreasure alloc]initWithFrame:CGRectMake(0, 0, SCREEN_WIDTH, SCREEN_HEIGHT)];
...
{
    if(!_sexView){
        _sexView = [[TribeOrganizerOrganizer alloc]initWithFrame:CGRectMake(0, 0, SCREEN_WIDTH, SCREEN_HEIGHT)];
//        _sexView.hidden = YES;
    }
    return _sexView;
}
