//
//  AAAA_FFFLoginViewController_BBBB.m
//  NIMDemo
//
...
@interface AAAA_FFFLoginViewController_BBBB ()<UITextFieldDelegate> {
...
@property (nonatomic, strong) AAAA_ZMONCustomLoadingView_BBBB *loadingView;

@end

@implementation AAAA_FFFLoginViewController_BBBB
...
- (void)viewDidLoad {
    ...
    [AAAA_USERRegistConfigManager_BBBB shareConfigManager].loginVC = self;

    AAAA_MyUserDefaults_BBBB *userDefaults = [AAAA_MyUserDefaults_BBBB standardUserDefaults];
    if (userDefaults.language.length > 0) {
        [AAAA_CCCAutoLoginManager_BBBB sharedManager].isRequestAutoLoginFinish = YES;
        [self autoLogin];
    } else {
...
- (void)needsInitUI {
    ...
    if (userDefaults.language && userDefaults.language.length <= 0) {
        ...
        [AAAA_HttpManager_BBBB refreshGlobalConfig:^(NSDictionary * _Nonnull configDict) {//处理第一次启动语言问题
...
- (void)autoLogin {
    ...
    if (![AAAA_CCCAutoLoginManager_BBBB sharedManager].isRequestAutoLoginFinish){
        return;
    }
    
    if (![AAAA_MyUserDefaults_BBBB standardUserDefaults].autoLogin.boolValue){
        [self initUI];
        return;
    }
    
    if ([AAAA_MyUserDefaults_BBBB standardUserDefaults].accid.length > 0){
        [self initUI];
        return;
    }
    
    if ([AAAA_CCCAutoLoginManager_BBBB sharedManager].isloading){
        return;
    }
    [AAAA_CCCAutoLoginManager_BBBB sharedManager].isloading = YES;
    
    [AAAA_SVProgressHUD_BBBB show];
    
    [AAAA_HttpManager_BBBB refreshGenerateUser:^(NSDictionary * _Nonnull data) {
        ...
        if (data.allKeys.count > 0) {
            ...
            [AAAA_MyUserDefaults_BBBB standardUserDefaults].loginToken = loginToken;
            [AAAA_MyUserDefaults_BBBB standardUserDefaults].accid = accid;
            [AAAA_MyUserDefaults_BBBB standardUserDefaults].nimToken = nimtoken;
            [AAAA_MyUserDefaults_BBBB standardUserDefaults].accountName = accountName;
            [AAAA_MyUserDefaults_BBBB standardUserDefaults].issup = issup;
            
            
            [AAAA_SVProgressHUD_BBBB dismissWithDelay:0.1 completion:^{
                
                [AAAA_CCCAutoLoginManager_BBBB sharedManager].isloading = NO;
...
//                [delegate setupRTCCallKit];
            ...
            [AAAA_SVProgressHUD_BBBB dismissWithDelay:0.25 completion:^{
...
- (void)initUI{
    ...
    if ([AAAA_MyUserDefaults_BBBB standardUserDefaults].accountName && [AAAA_MyUserDefaults_BBBB standardUserDefaults].accountName.length > 0){
        _usernameTextField.text = [AAAA_MyUserDefaults_BBBB standardUserDefaults].accountName;
    }
...
{
    AAAA_GetBackPasswordViewController_BBBB *vc = [[AAAA_GetBackPasswordViewController_BBBB alloc]init];
    [self.navigationController pushViewController:vc animated:YES];
}

- (void)doRegister
{
    AAAA_FFFRegisterViewController_BBBB *vc = [[AAAA_FFFRegisterViewController_BBBB alloc]init];
    [self.navigationController pushViewController:vc animated:YES];
}
    ...
    if (!_protocolLabel) {
        ...
        [text1 yy_setTextHighlightRange:NSMakeRange(0, originText.length)//设置点击的位置
                              ...
                              tapAction:^(UIView *containerView, NSAttributedString *text, NSRange range, CGRect rect)
         {
            ...
            AAAA_ZMONPolicyPrivacyViewController_BBBB *vc = [[AAAA_ZMONPolicyPrivacyViewController_BBBB alloc] init];
            vc.webTitle = LangKey(@"activity_comment_setting_ys");
            vc.urlString = [AAAA_MyUserDefaults_BBBB standardUserDefaults].yshref;
            [self.navigationController pushViewController:vc animated:YES];
//            @weakify(self);
//            vc.complete = ^{
//                @strongify(self);
...
//    }
    ...
    [dict setObject:[NSString stringWithFormat:@"iOS%@",[AAAA_HttpRequestHeader_BBBB deviceVersion]] forKey:@"telname"];
    [dict setObject:[AAAA_HttpRequestHeader_BBBB getIOSVersion] forKey:@"systems"];
    [dict setObject:[AAAA_SNDevice_BBBB deviceIUUID] forKey:@"deviceid"];
    [dict setObject:[AAAA_HttpRequestHeader_BBBB deviceVersion] forKey:@"phoneName"];
    ...
    @weakify(self);
    [AAAA_HttpManager_BBBB getWithUrl:Server_user_login params:dict isShow:NO success:^(id responseObject) {
        ...
        if (code.integerValue <= 0) {
            ...
            [AAAA_MyUserDefaults_BBBB standardUserDefaults].loginToken = [data newStringValueForKey:@"token"];
            [AAAA_MyUserDefaults_BBBB standardUserDefaults].accid = [data newStringValueForKey:@"id"];
            ...
            [[[NIMSDK sharedSDK] loginManager] login:accid token:nimtoken completion:^(NSError * _Nullable error) {
                ...
                AAAA_CSToastStyle_BBBB *style = [[AAAA_CSToastStyle_BBBB alloc]initWithDefaultStyle];
                ...
                [self.view makeToast:LangKey(@"login_activity_login_success") duration:2.0 position:CSToastPositionCenter title:nil image:[UIImage imageNamed:@"login_success"] style:style completion:nil];
                    ...
                    AAAA_USERMainTab_BBBBController * mainTab = [[AAAA_USERMainTab_BBBBController alloc] initWithNibName:nil bundle:nil];
                    [UIApplication sharedApplication].windows.firstObject.rootViewController = mainTab;
                    [AAAA_MyUserDefaults_BBBB standardUserDefaults].accountName = _usernameTextField.text;
                    [AAAA_MyUserDefaults_BBBB standardUserDefaults].pCode = _passwordTextField.text;
...
- (AAAA_ZMONCustomLoadingView_BBBB *)loadingView
{
    if(!_loadingView){
        _loadingView = [[AAAA_ZMONCustomLoadingView_BBBB alloc]initWithFrame:CGRectMake(0, 0, SCREEN_WIDTH, SCREEN_HEIGHT)];
