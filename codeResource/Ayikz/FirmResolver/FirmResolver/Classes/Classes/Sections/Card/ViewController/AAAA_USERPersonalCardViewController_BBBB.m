...
@interface AAAA_USERPersonalCardViewController_BBBB ()<NIMUserManagerDelegate,AAAA_HMDataPicker_BBBBDelegate,AAAA_USERReportDelegate_BBBBDelegate,AAAA_ZMONReport_BBBBDelegate>

@property (nonatomic,strong) AAAA_FFFCommonTableDelegate_BBBB *delegator;
...
@property (nonatomic, strong) AAAA_ZMONReport_BBBBUserView *reprotView;
@property (nonatomic, strong) AAAA_FFFRemarksView_BBBB *changeRemarksView;
...
@implementation AAAA_USERPersonalCardViewController_BBBB
...
//    [self setUpNav];
    ...
    [AAAA_HttpManager_BBBB getWithUrl:Server_user_detail params:dict isShow:NO success:^(id responseObject) {
...
//    [AAAA_SVProgressHUD_BBBB show];
    ...
    [AAAA_HttpManager_BBBB getWithUrl:Server_group_getgroups params:nil isShow:NO success:^(id responseObject) {
        [self.loadingView animationClose];
//        [AAAA_SVProgressHUD_BBBB dismissWithCompletion:^{
            NSDictionary *resultDict = (NSDictionary *)responseObject;
            NSString *code = [resultDict newStringValueForKey:@"code"];
            if (code.integerValue == 0) {
                ...
                AAAA_HMDataPicker_BBBB *dataPick = [[AAAA_HMDataPicker_BBBB alloc] initWithDelegate:self dataDict:dataDict selectedDict:nil jsonNode:@"name"];
                dataPick.tag = 500;
                [dataPick show];
            }
//        }];
...
#pragma mark - AAAA_HMDataPicker_BBBBDelegate
...
- (void)uploadImage:(UIImage *)image complete:(void(^)(NSString *urlString ))complete {
    ...
    NSString *fileName = [AAAA_FFFKitFileLocationHelper_BBBB genFilenameWithExt:@"jpg"];
    NSString *filePath = [[AAAA_FFFKitFileLocationHelper_BBBB getAppDocumentPath] stringByAppendingPathComponent:fileName];
...
-(void)addFriend{
    ...
    if (_teamSetingConfig != nil) {
        ...
        if (canAddFriend.integerValue <= 0) {
            [AAAA_SVProgressHUD_BBBB showMessage:LangKey(@"please_contact_your_administrator")];
            return;
        }
    }
...
-(void)sendAddFriendRequest{
    NSString *tempVerificationInfo = [AAAA_MyUserDefaults_BBBB standardUserDefaults].tempVerificationInfo;
...
-(void)setingGroup{
    ...
    [AAAA_HttpManager_BBBB getWithUrl:Server_group_addgroupuser params:dict isShow:YES success:^(id responseObject) {
        NSDictionary *resultDict = (NSDictionary *)responseObject;
...
- (AAAA_FFFRemarksView_BBBB *)changeRemarksView
{
    if(!_changeRemarksView){
        _changeRemarksView = [[AAAA_FFFRemarksView_BBBB alloc]initWithFrame:CGRectMake(0, 0, SCREEN_WIDTH, SCREEN_HEIGHT)];
...
- (AAAA_ZMONReport_BBBBUserView *)reprotView
{
    if(!_reprotView){
        _reprotView  = [[AAAA_ZMONReport_BBBBUserView alloc]initWithFrame:CGRectMake(0, 0, SCREEN_WIDTH, SCREEN_HEIGHT)];
