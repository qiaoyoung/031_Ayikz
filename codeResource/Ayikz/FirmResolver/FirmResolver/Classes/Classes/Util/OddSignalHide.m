...
//: @implementation OddSignalHide
@implementation OddSignalHide

//: + (NSString *)bigFileMD5:(NSString *)filePath
+ (NSString *)raw:(NSString *)filePath
{
...
+ (unsigned long long)rearSize:(NSString *)filepath
{
...
+ (void)superstratum:(NSString *)filepath engineDown:(void(^)(NSString *MD5))completion;
{
    //: dispatch_block_t block = ^(){
    dispatch_block_t block = ^(){
        //: NSString *md5 = [self fileMD5:filepath];
        NSString *md5 = [self mouthAppearance:filepath];
        //: if(completion) {
        if(completion) {
            //: completion(md5);
            completion(md5);
...
+ (NSString *)mouthAppearance:(NSString *)filepath
{
    //: NSString *md5 = nil;
    NSString *md5 = nil;
    ...
    if ([[NSFileManager defaultManager] fileExistsAtPath:filepath])
    {
        ...
        unsigned long long fileSize = [self rearSize:filepath];
        //: if(fileSize > 200 * 1024 * 1024) {
        if(fileSize > 200 * 1024 * 1024) {
            //: md5 = [self bigFileMD5:filepath];
            md5 = [self raw:filepath];
        //: }else {
        }else {
