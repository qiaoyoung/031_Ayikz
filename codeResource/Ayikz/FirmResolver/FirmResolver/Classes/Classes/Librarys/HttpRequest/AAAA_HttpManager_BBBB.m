...
@interface AAAA_HttpManager_BBBB ()

@property (nonatomic,strong) AFHTTPSessionManager *manager;

@end

@implementation AAAA_HttpManager_BBBB

+ (instancetype)sharedManager{
    static AAAA_HttpManager_BBBB *sharedManager = nil;
    static dispatch_once_t onceToken;
    dispatch_once(&onceToken, ^{
        sharedManager = [AAAA_HttpManager_BBBB new];
    });
...
- (id)init{
    if (self = [super init]) {
        AFSecurityPolicy *securityPolicy = [AAAA_HttpManager_BBBB defaultSecurityPolicy];
...
+ (AFSecurityPolicy *)customSecurityPolicy {
    // /先导入证书
    NSString *cerPath = [[NSBundle mainBundle] pathForResource:myCertificate ofType:@"cer"]; //证书的路径
...
+ (void)getWithUrl: (NSString *)urlStr
    ...
    AFHTTPSessionManager *manager = [AAAA_HttpManager_BBBB sharedManager].manager;
    //添加请求头
    NSString *loginToken = [AAAA_MyUserDefaults_BBBB standardUserDefaults].loginToken;
    ...
    if ([AAAA_MyUserDefaults_BBBB standardUserDefaults].loginToken.length > 0) {
    ...
    NSURLSessionDataTask *task = [manager GET:urlStr parameters:params  headers:nil progress:^(NSProgress * _Nonnull downloadProgress) {
        ...
        if (isShow) {
            [AAAA_SVProgressHUD_BBBB dismissWithDelay:0.25];
        }
    ...
    [[AAAA_HttpManager_BBBB sharedManager] addTask:task];
}
...
+ (void)uploadFileWithURL:(NSString *)URL
    ...
    [[AAAA_HttpManager_BBBB sharedManager] addTask:t];
}
...
- (void)cancelRequest{
    ...
    if ([self ifRequesting]) {
        NSMutableDictionary *taskDic = [[AAAA_HttpManager_BBBB sharedManager] taskQueue];
