...
//: #5D5F66
Byte colorMakeSettings[] = {64, 7, 81, 5, 185, 116, 134, 149, 134, 151, 135, 135, 158};

//: NSUserDefaultLanguage
Byte widgetNabName[] = {78, 21, 68, 8, 92, 158, 24, 185, 146, 151, 153, 183, 169, 182, 136, 169, 170, 165, 185, 176, 184, 144, 165, 178, 171, 185, 165, 171, 169, 26};

//: 完成
Byte layoutGlobeGiftedUtility[] = {59, 6, 35, 7, 236, 97, 41, 8, 209, 175, 9, 171, 179, 76};

//: yyyy-MM-dd
Byte appAvoidBlinkEvent[] = {1, 10, 68, 4, 189, 189, 189, 189, 113, 145, 145, 113, 168, 168, 156};

//: zh_CN
Byte appThoroughlyLogger[] = {43, 5, 94, 9, 160, 18, 190, 231, 146, 216, 198, 189, 161, 172, 250};

//: 0d8bf5
Byte appFromPreference[] = {98, 6, 91, 6, 15, 56, 139, 191, 147, 189, 193, 144, 85};

//: eeeeee
Byte spacingPastFatiguePreference[] = {73, 6, 27, 11, 32, 143, 79, 251, 242, 82, 56, 128, 128, 128, 128, 128, 128, 232};

//: 取消
Byte colorTendedDevice[] = {19, 6, 24, 13, 237, 209, 15, 13, 180, 173, 161, 248, 1, 253, 167, 174, 254, 206, 160, 116};
...
//: @interface EnableExpertDisplayGetView (){
@interface EnableExpertDisplayGetView (){
...
@property (nonatomic,retain) UIView *between;

//: @property (nonatomic,retain) UILabel *yearLabel;
@property (nonatomic,retain) UILabel *without;
//: @property (nonatomic, strong) UIView *bgView;
@property (nonatomic, strong) UIView *controlAll;

//: @end
@end

//: @implementation EnableExpertDisplayGetView
@implementation EnableExpertDisplayGetView
//: @synthesize delegate = _delegate;
@synthesize manHiveTransformerses = _womanSize;
//: @synthesize datePicker;
@synthesize circlePicker = member;
//: @synthesize yearLabel;
@synthesize without = phase;
//: @synthesize blackBackgroundButton = _blackBackgroundButton;
@synthesize quick = _offYear;
//: @synthesize animationView = _animationView;
@synthesize between = _mainAntiView;
...
- (void)than:(NSDate*)date{
    ...
    NSString *stringYMD = [NSDate fileTeamMobile:date challenge:StringFromVeteranData(appAvoidBlinkEvent)];
    ...
    //    }
    //: yearLabel.text = stringYMD;
    phase.text = stringYMD;
    //: yearLabel.font = [UIFont boldSystemFontOfSize:24];
    phase.font = [UIFont boldSystemFontOfSize:24];
}
...
- (id)initWithFrame:(CGRect)frame{
    ...
    if (self) {
        ...
        _mainAntiView = [[UIView alloc]initWithFrame:CGRectMake(0, [UIApplication sharedApplication].windows.firstObject.frame.size.height+50, [UIApplication sharedApplication].windows.firstObject.frame.size.width, 190 +50+(({UIEdgeInsets i; if(@available(iOS 11.0, *)) {i = ({UIWindow *win = nil; if (@available(iOS 13.0, *)) { NSSet *connectedScenes = [UIApplication sharedApplication].connectedScenes; for (UIScene *scene in connectedScenes) { if ([scene isKindOfClass:[UIWindowScene class]] && scene.activationState == UISceneActivationStateForegroundActive) { UIWindowScene *windowScene = (UIWindowScene *)scene; for (UIWindow *w in windowScene.windows) { if (w.isKeyWindow) { win = w; break; } } if (!win) { win = windowScene.windows.firstObject; } if (win) { break; } } } } if (!win) { for (UIWindow *w in [UIApplication sharedApplication].windows) { if (w.isKeyWindow) { win = w; break; } } if (!win) { win = [UIApplication sharedApplication].windows.firstObject; } } if (!win) { if ([[UIApplication sharedApplication] delegate] && [[[UIApplication sharedApplication] delegate] respondsToSelector:@selector(factoryLarging)]) { win = [[[UIApplication sharedApplication] delegate] window]; } } win;}).safeAreaInsets;} else {i = UIEdgeInsetsZero;} i;}).bottom))];
        //: _animationView.backgroundColor = [UIColor whiteColor];
        _mainAntiView.backgroundColor = [UIColor whiteColor];
        //: _animationView.userInteractionEnabled = YES;
        _mainAntiView.userInteractionEnabled = YES;
        //: [self addSubview:_animationView];
        [self addSubview:_mainAntiView];
        ...
        member = [[UIDatePicker alloc]initWithFrame:CGRectMake(0,50, [UIApplication sharedApplication].windows.firstObject.frame.size.width, 190)];
        //: [datePicker addTarget:self action:@selector(datePickerValueChanged) forControlEvents:UIControlEventValueChanged];
        [member addTarget:self action:@selector(factorBasic) forControlEvents:UIControlEventValueChanged];
        //: [datePicker setDate:[NSDate date]];
        [member setDate:[NSDate date]];
        //: datePicker.datePickerMode = UIDatePickerModeDate;
        member.datePickerMode = UIDatePickerModeDate;
        //: [_animationView addSubview:datePicker];
        [_mainAntiView addSubview:member];

        //: NSString *lang = [[NSUserDefaults standardUserDefaults] objectForKey:@"NSUserDefaultLanguage"];
        NSString *lang = [[NSUserDefaults standardUserDefaults] objectForKey:StringFromVeteranData(widgetNabName)];
        //: if (lang.length <= 0) {
        if (lang.length <= 0) {
            //: lang = @"zh_CN";
            lang = StringFromVeteranData(appThoroughlyLogger);
        }
        ...
        if (@available(iOS 14.0, *)) {
            ...
            member.preferredDatePickerStyle = UIDatePickerStyleWheels;
            ...
            member.frame = CGRectMake(0,50, [UIApplication sharedApplication].windows.firstObject.frame.size.width, 190);
        }
...
- (void)buttonned:(id)sender{
    ...
    if ((currentClass == touchstoneClass) && [self.manHiveTransformerses respondsToSelector:@selector(futurismPending)]) {
        ...
        [self.manHiveTransformerses futurismPending];
    }
    //: if ((currentClass == delegateClass) && [self.delegate respondsToSelector:@selector(datePick:doneWithDate:)]) {
    if ((currentClass == touchstoneClass) && [self.manHiveTransformerses respondsToSelector:@selector(rateDetail:patent:)]) {
        //: [self.delegate datePick:self doneWithDate:datePicker.date];
        [self.manHiveTransformerses rateDetail:self patent:member.date];
    }
    ...
    [UIView animateWithDuration:0.3 animations:^{
        ...
        _mainAntiView.frame = CGRectOffset(self.frame, 0, self.frame.size.height);
    //: } completion:^(BOOL finished) {
    } completion:^(BOOL finished) {
...
+ (id)historical:(id<EnableExpertDisplayGetViewDelegate>)delegate decide:(NSDate*)minDate shadow:(NSDate*)maxDate speed:(NSDate*)showDate{
    //: EnableExpertDisplayGetView *pickerView = [[EnableExpertDisplayGetView alloc]initWithFrame:CGRectMake(0, 0, [UIApplication sharedApplication].windows.firstObject.frame.size.width, [UIApplication sharedApplication].windows.firstObject.frame.size.height)];
    EnableExpertDisplayGetView *pickerView = [[EnableExpertDisplayGetView alloc]initWithFrame:CGRectMake(0, 0, [UIApplication sharedApplication].windows.firstObject.frame.size.width, [UIApplication sharedApplication].windows.firstObject.frame.size.height)];
    ...
    if (minDate) {
        ...
        [pickerView.circlePicker setMinimumDate:minDate];
    }
    //: if (maxDate) {
    if (maxDate) {
        //: [pickerView.datePicker setMaximumDate:maxDate];
        [pickerView.circlePicker setMaximumDate:maxDate];
    }
    //: if (showDate) {
    if (showDate) {
        //: [pickerView.datePicker setDate:showDate];
        [pickerView.circlePicker setDate:showDate];
    }
    //: [pickerView changeDelegate:delegate];
    [pickerView be:delegate];
...
+ (void)owner:(UIView*)view segmentDetail:(id<EnableExpertDisplayGetViewDelegate>)delegate date:(NSDate*)minDate itemDate:(NSDate*)maxDate write:(NSDate *)showDate{
    ...
    //: EnableExpertDisplayGetView *pickerView = [[EnableExpertDisplayGetView alloc]initWithFrame:CGRectMake(0, 0, [UIApplication sharedApplication].windows.firstObject.frame.size.width, view.frame.size.height)];
    EnableExpertDisplayGetView *pickerView = [[EnableExpertDisplayGetView alloc]initWithFrame:CGRectMake(0, 0, [UIApplication sharedApplication].windows.firstObject.frame.size.width, view.frame.size.height)];
    //: pickerView.tag = 2006021272;
    pickerView.tag = 2006021272;
    //: if (minDate) {
...
- (void)composition{
    ...
    UIView *upVeiw = [[UIView alloc]initWithFrame:CGRectMake(0, CGRectGetMinY(member.frame)-50, [UIApplication sharedApplication].windows.firstObject.frame.size.width, 50)];
    //: upVeiw.backgroundColor = [UIColor whiteColor];
    upVeiw.backgroundColor = [UIColor whiteColor];
    //: [_animationView addSubview:upVeiw];
    [_mainAntiView addSubview:upVeiw];
    //: upVeiw.layer.borderWidth = 0.5f;
    upVeiw.layer.borderWidth = 0.5f;
    //: upVeiw.layer.borderColor = [UIColor colorWithHexString:@"eeeeee"].CGColor;
    upVeiw.layer.borderColor = [UIColor tin:StringFromVeteranData(spacingPastFatiguePreference)].CGColor;

    //左边的取消按钮
    //: cancelButton = [UIButton buttonWithType:UIButtonTypeCustom];
    remark = [UIButton buttonWithType:UIButtonTypeCustom];
    //: cancelButton.frame = CGRectMake(12, 0, 50, 50);
    remark.frame = CGRectMake(12, 0, 50, 50);
    //: [cancelButton setTitle:@"取消".nim_localized forState:UIControlStateNormal];
    [remark setTitle:StringFromVeteranData(colorTendedDevice).totalroStructure forState:UIControlStateNormal];
    //: cancelButton.backgroundColor = [UIColor clearColor];
    remark.backgroundColor = [UIColor clearColor];
    //: cancelButton.titleLabel.font = [UIFont systemFontOfSize:16];
    remark.titleLabel.font = [UIFont systemFontOfSize:16];
    //: [cancelButton setTitleColor:[UIColor colorWithHexString:@"0d8bf5"] forState:UIControlStateNormal];
    [remark setTitleColor:[UIColor tin:StringFromVeteranData(appFromPreference)] forState:UIControlStateNormal];
    //: [cancelButton addTarget:self action:@selector(leftButtonClicked:) forControlEvents:UIControlEventTouchUpInside];
    [remark addTarget:self action:@selector(stills:) forControlEvents:UIControlEventTouchUpInside];
    //: [upVeiw addSubview:cancelButton];
    [upVeiw addSubview:remark];
    ...
    forthButton.frame = CGRectMake([UIScreen mainScreen].bounds.size.width - 62, 0, 50, 50);
    //: [chooseButton setTitle:@"完成".nim_localized forState:UIControlStateNormal];
    [forthButton setTitle:StringFromVeteranData(layoutGlobeGiftedUtility).totalroStructure forState:UIControlStateNormal];
    ...
    forthButton.titleLabel.font = [UIFont systemFontOfSize:16];
    //: [chooseButton setTitleColor:[UIColor colorWithHexString:@"0d8bf5"] forState:UIControlStateNormal];
    [forthButton setTitleColor:[UIColor tin:StringFromVeteranData(appFromPreference)] forState:UIControlStateNormal];
    //: [chooseButton addTarget:self action:@selector(rightButtonClicked:) forControlEvents:UIControlEventTouchUpInside];
    [forthButton addTarget:self action:@selector(buttonned:) forControlEvents:UIControlEventTouchUpInside];
    //: [upVeiw addSubview:chooseButton];
    [upVeiw addSubview:forthButton];

    //: self.titleL = [[UILabel alloc] initWithFrame:CGRectMake(62, 10, [[UIScreen mainScreen] bounds].size.width-124, 30)];
    self.well = [[UILabel alloc] initWithFrame:CGRectMake(62, 10, [[UIScreen mainScreen] bounds].size.width-124, 30)];
    //: [upVeiw addSubview:_titleL];
    [upVeiw addSubview:_well];
    //: _titleL.textColor = [UIColor colorWithHexString:@"#5D5F66"];
    _well.textColor = [UIColor tin:StringFromVeteranData(colorMakeSettings)];
    //: _titleL.font = [UIFont systemFontOfSize:16];
    _well.font = [UIFont systemFontOfSize:16];
    //: _titleL.textAlignment = NSTextAlignmentCenter;
    _well.textAlignment = NSTextAlignmentCenter;
    //: _titleL.text = _titleString;
    _well.text = _runLetter;
    ...
    if([[[UIDevice currentDevice] systemVersion] floatValue] >= 7.0){
        ...
        member.backgroundColor = [UIColor whiteColor];
    }
...
Byte * VeteranDataToCache(Byte *data) {
    int citedWarning = data[0];
    int hostageMore = data[1];
    Byte lowingOccasional = data[2];
    int supportFormat = data[3];
    if (!citedWarning) return data + supportFormat;
    for (int i = supportFormat; i < supportFormat + hostageMore; i++) {
        int value = data[i] - lowingOccasional;
        if (value < 0) {
            value += 256;
        }
        data[i] = value;
    ...
    data[0] = 0;
    data[supportFormat + hostageMore] = 0;
    return data + supportFormat;
}

NSString *StringFromVeteranData(Byte *data) {
    return [NSString stringWithUTF8String:(char *)VeteranDataToCache(data)];
}
