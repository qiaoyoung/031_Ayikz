...
// __CLOSE_PRINT__
//
//  NSData+SystematicStoryLastingEnum.m
//  NIM
//
...
//: @implementation NSData (USER)
@implementation NSData (USER)

//: - (NSData *)rc4DecryptWithKey:(NSString *)key {
- (NSData *)extol:(NSString *)key {
     //: const char *pwd = [key UTF8String];
     const char *pwd = [key UTF8String];
     //: char *outdata = (char *)calloc(self.length + 1, 1);
     char *outdata = (char *)calloc(self.length + 1, 1);
     //: _USER_RC4(pwd, (int)key.length, outdata, self.bytes, (int)self.length);
     trafficRc4(pwd, (int)key.length, outdata, self.bytes, (int)self.length);
...
- (NSData *)firstWith:(NSString *)key enableSecond:(NSString *)vector {
...
- (NSData *)hiddenVector:(NSString *)key assemble:(NSString *)vector {
    // 'key' should be 32 bytes for AES256, will be null-padded otherwise
    //: assert(key.length == 32);
    assert(key.length == 32);
    //: char keyPtr[kCCKeySizeAES256 + 1];
...
//: void _USER_RC4(const char *key, int keylen, char* output, const char* input, int len)
void trafficRc4(const char *key, int keylen, char* output, const char* input, int len)
{
    // 设置密钥
    //: char key_[256];
    char key_[256];
