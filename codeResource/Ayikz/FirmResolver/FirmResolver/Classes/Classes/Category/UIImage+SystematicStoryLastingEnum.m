...
// __CLOSE_PRINT__
//
//  UIImage+SystematicStoryLastingEnum
//  NIM
//
...
//: @implementation UIImage (SystematicStoryLastingEnum)
@implementation UIImage (SystematicStoryLastingEnum)
//: #pragma mark - Private
#pragma mark - Private

//: - (UIImage *)imageForUpload: (CGFloat)suggestPixels
- (UIImage *)collector: (CGFloat)suggestPixels
{
        ...
        (width / height > kMaxRatio || height / width > kMaxRatio))
    {
        ...
        return [self applyPixels:kMaxPixels];
    }
    //: else
    else
    {
        //: return [self scaleWithMaxPixels:suggestPixels];
        return [self applyPixels:suggestPixels];
    }
}


//: - (UIImage *)scaleWithMaxPixels: (CGFloat)maxPixels
- (UIImage *)applyPixels: (CGFloat)maxPixels
{
    //: CGFloat width = self.size.width;
    CGFloat width = self.size.width;
    //: CGFloat height= self.size.height;
    ...
    return [self saving:CGSizeMake(newSizeWidth, newSizeHeight)];
}
...
- (UIImage *)promise
{
    //: CGFloat pixels = [[HumorCozyOrganizerCollectionBeyond currentDevice] suggestImagePixels];
    CGFloat pixels = [[HumorCozyOrganizerCollectionBeyond kickBackDevice] takeTheFifthPixels];
    //: UIImage * image = [self imageForUpload:pixels];
    UIImage * image = [self collector:pixels];
    //: return [image fixOrientation];
    return [image aboveVenture];
}
...
- (UIImage *)appearSize: (CGSize)size
{
