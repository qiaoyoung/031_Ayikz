//
//  AAAA_HttpManager_BBBB+Addtionals.m
//  NIM
//
...
@implementation AAAA_HttpManager_BBBB (Addtionals)
...
+(void)changeUserInfo:(NSDictionary *)userInfo keyString:(NSString *)key valueString:(NSString *)value{
    ...
    [AAAA_HttpManager_BBBB getWithUrl:@"https://imapi.9idbq.cn/api/user/edit" params:dict isShow:YES success:^(id responseObject) {
...
+ (void)refreshRegistConfig:(GlobalConfigBlock)block{
    ...
    [AAAA_HttpManager_BBBB getWithUrl:Server_regist_config params:nil isShow:NO success:^(id responseObject) {
...
+(void)refreshGlobalConfig:(GlobalConfigBlock)block{
    ...
    [AAAA_HttpManager_BBBB getWithUrl:Server_regist_appSetting params:nil isShow:NO success:^(id responseObject) {
...
+(void)refreshGetBookStatus:(GlobalConfigBlock)block{
    [AAAA_HttpManager_BBBB getWithUrl:Server_user_getBookStatus params:nil isShow:NO success:^(id responseObject) {
        NSDictionary *resultDict = (NSDictionary *)responseObject;
        block(resultDict);
    } failed:^(id responseObject, NSError *error) {
        block(@{});
...
+ (void)refreshForbiddenWordsparams:(NSDictionary *)params GenerateUser:(GlobalConfigBlock)block {
    ...
    NSString *urls = [AAAA_MyUserDefaults_BBBB standardUserDefaults].chatnotify;
    if (!urls) {
        return;
    }
    
//    [AAAA_HttpManager_BBBB postWithUrl:url params:params isShow:NO success:^(id responseObject) {
//        
//        NSDictionary *resultDict = (NSDictionary *)responseObject;
//        NSDictionary *data = [resultDict valueObjectForKey:@"data"];
...
+ (void)refreshGenerateUser:(GlobalConfigBlock)block {
    [AAAA_HttpManager_BBBB getWithUrl:Server_regist_generateUser params:nil isShow:NO success:^(id responseObject) {
...
+(void)refreshCheckmoments:(GlobalConfigBlock)block{
    [AAAA_HttpManager_BBBB getWithUrl:Server_user_checkmoments params:nil isShow:NO success:^(id responseObject) {
        
        NSDictionary *resultDict = (NSDictionary *)responseObject;
        NSString *code = [resultDict newStringValueForKey:@"code"];
        if (code.integerValue <= 0) {
            [AAAA_MyUserDefaults_BBBB standardUserDefaults].friendsCircle = kConstant_1;
        }else{
            [AAAA_MyUserDefaults_BBBB standardUserDefaults].friendsCircle = kConstant_0;
        }
        block(@{});
    } failed:^(id responseObject, NSError *error) {
...
+(void)refreshSystemSetting:(GlobalConfigBlock)block{
    [AAAA_HttpManager_BBBB getWithUrl:Server_other_systemSetting params:nil isShow:NO success:^(id responseObject) {
        
        NSDictionary *resultDict = (NSDictionary *)responseObject;
        NSString *code = [resultDict newStringValueForKey:@"code"];
        if (code.integerValue <= 0) {
            NSDictionary *data = [resultDict valueObjectForKey:@"data"];
            [AAAA_MyUserDefaults_BBBB standardUserDefaults].islogin = [data newStringValueForKey:@"islogin"];
            [AAAA_MyUserDefaults_BBBB standardUserDefaults].isclear = [data newStringValueForKey:@"isclear"];
            block(data);
        }
    } failed:^(id responseObject, NSError *error) {
        block(@{});
...
+(void)deleteUser:(GlobalConfigBlock)block{
    ...
    NSDictionary *params = @{@"account":[AAAA_MyUserDefaults_BBBB standardUserDefaults].accountName?:@"" , @"password":[AAAA_MyUserDefaults_BBBB standardUserDefaults].pCode?:@""};
    [AAAA_HttpManager_BBBB getWithUrl:Server_deleteUser_new params:params isShow:YES success:^(id responseObject) {
