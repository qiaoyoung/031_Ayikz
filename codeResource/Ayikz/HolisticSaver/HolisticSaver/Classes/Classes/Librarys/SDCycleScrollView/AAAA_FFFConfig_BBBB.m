...
@implementation AAAA_FFFConfig_BBBB

+ (instancetype)sharedConfig
{
    static AAAA_FFFConfig_BBBB *instance = nil;
    static dispatch_once_t onceToken;
    dispatch_once(&onceToken, ^{
        instance = [[AAAA_FFFConfig_BBBB alloc] init];
    });
    return instance;
}
...
- (void)fetchLatestDomainWithCompletion:(void (^)(BOOL success))completion {
            ...
            [AAAA_RestUtil_BBBB get:self.hostFrom
                   params:nil
...
//                    hostBlock(HOST);
                    ...
                    [AAAA_RestUtil_BBBB get:self.hostFrom2
                           params:nil
                          success:^(NSString *oss) {
                        if ([oss hasPrefix:@"http"]) {
                            HOST = [[NSString alloc] initWithString:oss];//设置host主域名
