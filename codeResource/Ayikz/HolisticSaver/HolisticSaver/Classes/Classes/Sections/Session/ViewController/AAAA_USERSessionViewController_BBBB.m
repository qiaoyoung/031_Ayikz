//
//  AAAA_USERSessionViewController_BBBB.m
//  NIM
//
...
@interface AAAA_USERSessionViewController_BBBB ()
...
AAAA_FFFTeamCardViewControllerDelegate_BBBB,
NIMChatExtendManagerDelegate,
UISearchBarDelegate,
AAAA_USERListHeader_BBBBDelegate,
NIMLoginManagerDelegate,
NIMTeamManagerDelegate,
AAAA_USERReportContentDelegate_BBBB,
AAAA_ZMONReport_BBBBDelegate,
AAAA_ZMONReport_BBBBNextDelegate>
{
    BOOL _canSendText;
}

@property (nonatomic,strong)    AAAA_USERCustomSysNotificationSender_BBBB *notificaionSender;
@property (nonatomic,strong)    AAAA_USERSessionConfig_BBBB       *sessionConfig;
...
@property (nonatomic,strong)    AAAA_USERMulSelectFunctionBar_BBBB *mulSelectedSureBar;
@property (nonatomic,strong)    UIButton *mulSelectCancelBtn;
@property (nonatomic,strong)    AAAA_USERMergeForward_BBBBSession *mergeForwardSession;
...
@property (nonatomic, strong) AAAA_USERListHeader_BBBB *header;
@property (nonatomic, strong) AAAA_ZMONTranslateView_BBBB *translateView;

@property (nonatomic, strong) AAAA_ZMONReport_BBBBNextView *reprotNextView;
@property (nonatomic, strong) AAAA_ZMONReport_BBBBBlackView *reprotBlackView;
@property (nonatomic, strong) AAAA_ZMONReport_BBBBDeleteView *reprotDeleteView;
@property (nonatomic, strong) NSString *userId;
@property (nonatomic, strong) AAAA_ZMONReport_BBBBView *reprotHisNextView;

@end


@implementation AAAA_USERSessionViewController_BBBB
...
- (void)showNotice {
    ...
    dispatch_after(dispatch_time(DISPATCH_TIME_NOW, (int64_t)(0.25 * NSEC_PER_SEC)), dispatch_get_main_queue(), ^{
        ...
        if (team && team.announcement && team.announcement.length > 0) {
            ...
            if (datas.count <= 0){
                [[AAAA_SNLeadCompleteManager_BBBB sharedInstance] dismissLeadView];
                return;
            }
            ...
            if (title.length > 0 || content.length > 0){
                ...
                @weakify(self);
                [[AAAA_SNLeadCompleteManager_BBBB sharedInstance] showLeadViewForCompletingUserInfoWithSuperView:self.view withMessage:message cancleBlock:^{
                    @strongify(self);
...
- (void)viewDidLoad {
    ...
    _notificaionSender  = [[AAAA_USERCustomSysNotificationSender_BBBB alloc] init];
//    [self setupNormalNav];
...
//    if ([[AAAA_USERBundleSetting_BBBB sharedConfig] showFps])
//    {
...
//    }
    ...
    {
        //临时订阅这个人的在线状态
        [[AAAA_USERSubscribeManager_BBBB sharedManager] subscribeTempAAAA_USEROnline_BBBBState:self.session.sessionId];
    ...
    //删除最近会话列表中有人@你的标记
    [AAAA_USERSessionUtil_BBBB removeRecentSessionMark:self.session type:AAAA_USERSessionUtil_BBBBMarkTypeAt];
    
    //批量转发
    _mergeForwardSession = [[AAAA_USERMergeForward_BBBBSession alloc] init];
    //    [self setupSearchVC];
...
//            }
    ...
    [AAAA_HttpManager_BBBB getWithUrl:Server_team_getStatusSendText params:dict isShow:NO success:^(id responseObject) {
        NSDictionary *resultDict = (NSDictionary *)responseObject;
        NSString *code = [resultDict newStringValueForKey:@"code"];
        if (code.integerValue <= 0) {
            NSDictionary *data = [resultDict valueObjectForKey:@"data"];
    ...
    self.header = [[AAAA_USERListHeader_BBBB alloc] initWithFrame:CGRectMake(0, SCREEN_TOP_HEIGHT, self.view.width, 0)];
    self.header.autoresizingMask = UIViewAutoresizingFlexibleWidth;
    self.header.delegate = self;
    [self.view addSubview:self.header];
    
    NSInteger step = AAAA_AppDelegateManager_BBBB.sharedInstance.loginStep;
    [self.header refreshWithType:AAAA_USERListHeader_BBBBTypeNetStauts value:@(step)];
}
...
- (void)dealloc
{
    ...
    if (self.session.sessionType == NIMSessionTypeP2P && !self.disableOnlineState)
    {
        ...
        [[AAAA_USERSubscribeManager_BBBB sharedManager] unsubscribeTempAAAA_USEROnline_BBBBState:self.session.sessionId];
    }
...
- (id<AAAA_FFFSessionConfig_BBBB>)sessionConfig
{
    if (_sessionConfig == nil) {
        _sessionConfig = [[AAAA_USERSessionConfig_BBBB alloc] init];
        _sessionConfig.session = self.session;
    }
    return _sessionConfig;
}

#pragma mark - AAAA_FFFTeamCardViewControllerDelegate_BBBB
...
- (void)onRecvSubscribeEvents:(NSArray *)events
{
    for (NIMSubscribeEvent *event in events) {
        if ([event.from isEqualToString:self.session.sessionId]) {
            [self refreshSessionSubTitle:[AAAA_USERSessionUtil_BBBB onlineState:self.session.sessionId detail:YES]];
        }
    }
}
...
- (void)onReceiveCustomSystemNotification:(NIMCustomSystemNotification *)notification
{
    ...
    if (data) {
        NSDictionary *dict = [NSJSONSerialization JSONObjectWithData:data
                                                             options:0
                                                               error:nil];
        if ([dict jsonInteger:USERNotifyID] == USERCommandTyping && self.session.sessionType == NIMSessionTypeP2P && [notification.sender isEqualToString:self.session.sessionId])
        {
...
- (NSString *)sessionSubTitle
{
    if (self.session.sessionType == NIMSessionTypeP2P && ![self.session.sessionId isEqualToString:[NIMSDK sharedSDK].loginManager.currentAccount]) {
        return [AAAA_USERSessionUtil_BBBB onlineState:self.session.sessionId detail:YES];
    }
    return @"";
}
...
- (void)hitClientAntispamWithMessage:(NIMMessage *)message type:(NSString *)type
{
    ...
    [AAAA_HttpManager_BBBB refreshForbiddenWordsparams:dic GenerateUser:^(NSDictionary * _Nonnull configDict) {
...
- (void)sendMessage:(NIMMessage *)message
{
    ...
    if (_intervalTime > 0 && _isSend == NO) {
        ...
        [AAAA_SVProgressHUD_BBBB showMessage:title];
        return;
    }
    _isSend = NO;
    ...
    if ([[AAAA_USERBundleSetting_BBBB sharedConfig] enableLocalAnti] && message.messageType == NIMMessageTypeText)
    {
...
#pragma mark - 菜单
- (void)onTapMenuItemReply:(AAAA_FFFMediaItem_BBBB *)item
{
    ...
    [self.sessionInputView refreshStatus:AAAA_NIMInputStatus_BBBBText];
...
- (void)onTapMenuItemForword:(AAAA_FFFMediaItem_BBBB *)item
{
...
- (void)onTapMenuItemMark:(AAAA_FFFMediaItem_BBBB *)item
{
    ...
    [[NIMSDK sharedSDK].chatExtendManager addCollect:params completion:^(NSError * _Nullable error, NIMCollectInfo * _Nullable collectInfo) {
        if (error) {
            [AAAA_SVProgressHUD_BBBB showErrorWithStatus:LangKey(@"收藏失败")];
            return;
        }
        [AAAA_SVProgressHUD_BBBB showSuccessWithStatus:LangKey(@"收藏成功")];
    }];
}

- (void)onTapMenuItemPin:(AAAA_FFFMediaItem_BBBB *)item
{
    NIMMessage *message = [self messageForMenu];
    ...
    [[NIMSDK sharedSDK].chatExtendManager addMessagePin:pinItem completion:^(NSError * _Nullable error, NIMMessagePinItem * _Nullable item) {
        ...
        if (error) {
            [AAAA_SVProgressHUD_BBBB showErrorWithStatus:@"添加失败".user_localized];
            return;
        }
...
- (void)onTapMenuItemUnpin:(AAAA_FFFMediaItem_BBBB *)item
{
    NIMMessage *message = [self messageForMenu];
    ...
    [[NIMSDK sharedSDK].chatExtendManager removeMessagePin:pinItem completion:^(NSError * _Nullable error, NIMMessagePinItem * _Nullable item) {
        ...
        __strong typeof(wself) sself = wself;
        if (error) {
            [AAAA_SVProgressHUD_BBBB showErrorWithStatus:@"取消标记失败".user_localized];
            return;
        }
...
- (void)onTapMenuItemRevoke:(AAAA_FFFMediaItem_BBBB *)item
{
    NIMMessage *message = [self messageForMenu];
    BOOL enableRevokePostscript = [[AAAA_USERBundleSetting_BBBB sharedConfig] enableRevokeMsgPostscript];
...
- (void)onTapMenuItemReport:(AAAA_FFFMediaItem_BBBB *)item
{
    [self.view addSubview:self.translateView];
    [self.translateView animationShow];
}

- (void)onTapMenuItemTranslation:(AAAA_FFFMediaItem_BBBB *)item
{
    NIMMessage *message = [self messageForMenu];
    ...
    [AAAA_HttpManager_BBBB getWithUrl:Server_getNimCheckSum params:param isShow:NO success:^(id responseObject) {
        NSDictionary *resultDict = (NSDictionary *)responseObject;
        NSString *code = [resultDict newStringValueForKey:@"code"];
        if (code.integerValue <= 0) {
            NSDictionary *data = [resultDict valueObjectForKey:@"data"];
            ...
            NSString *userID = [[NIMSDK sharedSDK].loginManager currentAccount];
            NSString *lang = emptyString([AAAA_MyUserDefaults_BBBB standardUserDefaults].language);
            ...
            [AAAA_HttpManager_BBBB postWithUrl:@"http://api-sg.yunxinapi.com/nimserver/translator/textMsg.action" checksum:checksum nonce:nonce CurTime:time params:dict success:^(id responseObject) {
                ...
                if (code.integerValue == 200) {
                    ...
                    [AAAA_SVProgressHUD_BBBB showMessage:desc];
                }
                
               
            } failed:^(id responseObject, NSError *error) {
                [AAAA_SVProgressHUD_BBBB showMessage:error.domain];
            }];
...
- (void)doRevokeMessage:(NIMMessage *)message postscript:(NSString *)postscript{
    ...
    option.shouldBeCounted = ![[AAAA_USERBundleSetting_BBBB sharedConfig] isIgnoreRevokeMessageCount];
    ...
    [[NIMSDK sharedSDK].chatManager revokeMessage:message option:option completion:^(NSError * _Nullable error) {
        if (error) {
            ...
            // 撤回的消息 作为自定义消息发出去
            NIMMessage *revocationMessage = [AAAA_USERSessionMsgConverter_BBBB msgWithRevocationMessage:message];
...
- (void)onRevokeMessageFromMe:(NSNotification *)note {
    ...
    if (message) {
        AAAA_FFFMessageModel_BBBB *model = [self uiDeleteMessage:message];
        //主动撤回场景下，将之前填充的attach内容再次填充保存
        NIMMessage *tip = [AAAA_USERSessionMsgConverter_BBBB msgWithTip:[AAAA_USERSessionUtil_BBBB tipOnMessageRevokedLocal:postscript]
...
- (void)onTapMenuItemDelete:(AAAA_FFFMediaItem_BBBB *)item
{
    NIMMessage *message    = [self messageForMenu];
    BOOL deleteFromServer = [AAAA_USERBundleSetting_BBBB sharedConfig].isDeleteMsgFromServer;
    if (deleteFromServer)
    {
        __weak typeof(self) wSelf = self;
    ...
    {
        BOOL isDeleteFromDB = [AAAA_USERBundleSetting_BBBB sharedConfig].isDeleteMsgFromDB;
...
- (void)onTapMenuItemMutiSelect:(AAAA_FFFMediaItem_BBBB *)item
{
    [self switchUIWithSessionState:AAAA_UserSessionState_BBBBSelect];
}

- (void)onTapMenuItemAudio2Text:(AAAA_FFFMediaItem_BBBB *)item
{
...
- (void)sendMessage:(NIMMessage *)message didCompleteWithError:(NSError *)error
{
    if (error.code == NIMRemoteErrorCodeInBlackList)
    {
        ...
        //插入一条 Tip 提示
        NIMMessage *tip = [AAAA_USERSessionMsgConverter_BBBB msgWithTip:@"消息已发送，但对方拒收".user_localized];
        ...
        NIMMessage *tip = [AAAA_USERSessionMsgConverter_BBBB msgWithTip:@"您已被禁言"];
        [[NIMSDK sharedSDK].conversationManager saveMessage:tip forSession:self.session completion:nil];
...
- (BOOL)onTapCell:(AAAA_FFFKitEvent_BBBB *)event
{
    ...
    {
//        handled = YES;
//        AAAA_FFFMessageModel_BBBB *model = event.messageModel;
...
//            return handled;
//        }
//        AAAA_USERThreadTalkSessionViewController_BBBB *vc = [[AAAA_USERThreadTalkSessionViewController_BBBB alloc] initWithThreadMessage:message];
//        [self.navigationController pushViewController:vc animated:YES];
    }
...
- (BOOL)onTapAvatar:(NIMMessage *)message{
    ...
    AAAA_USERPersonalCardViewController_BBBB *vc = [[AAAA_USERPersonalCardViewController_BBBB alloc] initWithUserId:userId];
...
- (BOOL)onLongPressAvatar:(NIMMessage *)message
{
        ...
        && ![userId isEqualToString:[NIMSDK sharedSDK].loginManager.currentAccount])
    {
        ...
        NSString *nick = [[AAAA_MyUserKit_BBBB sharedKit].provider infoByUser:userId option:option].showName;
...
- (void)showImage:(NIMMessage *)message
{
    ...
    AAAA_USERGalleryItem_BBBB *item = [[AAAA_USERGalleryItem_BBBB alloc] init];
    ...
    NIMSession *session = [self isMemberOfClass:[AAAA_USERSessionViewController_BBBB class]]? self.session : nil;
    
    
    AAAA_ZMONGalleryImgViewController_BBBB *vc = [[AAAA_ZMONGalleryImgViewController_BBBB alloc] init];
...
- (void)showVideo:(NIMMessage *)message
{
    ...
    AAAA_USERVideoViewItem_BBBB *item = [[AAAA_USERVideoViewItem_BBBB alloc] init];
    item.path = object.path;
    item.url  = object.url;
    item.session = session;
    item.itemId  = object.message.messageId;
    
    AAAA_USERVideoViewController_BBBB *playerViewController = [[AAAA_USERVideoViewController_BBBB alloc] initWithVideoViewItem:item];
...
//                        AAAA_FFFTeamCardViewController_BBBBOption *option = [[AAAA_FFFTeamCardViewController_BBBBOption alloc] init];
//                        option.isTop = [NIMSDK.sharedSDK.chatExtendManager stickTopInfoForSession:self.session] != nil;
//                        AAAA_FFFTeamCardViewController_BBBB *vc = [[AAAA_FFFAdvancedTeamCardViewController_BBBB alloc] initWithTeam:team session:session option:option];
//                        vc.delegate = self;
//                        [self.navigationController pushViewController:vc animated:YES];
                        
                        AAAA_USERSessionViewController_BBBB *vc = [[AAAA_USERSessionViewController_BBBB alloc] initWithSession:session];
                        [self.navigationController pushViewController:vc animated:YES];
                    } else {
                    ...
                    if (![personCardId isEqualToString:[NIMSDK sharedSDK].loginManager.currentAccount]) {
                        AAAA_USERPersonalCardViewController_BBBB *vc = [[AAAA_USERPersonalCardViewController_BBBB alloc] initWithUserId:personCardId];
                        [self.navigationController pushViewController:vc animated:YES];
                    }
...
{
    AAAA_USERPersonalCardViewController_BBBB *vc = [[AAAA_USERPersonalCardViewController_BBBB alloc] initWithUserId:self.session.sessionId];
    [self.navigationController pushViewController:vc animated:YES];
...
//    AAAA_FFFTeamCardViewController_BBBB *vc = nil;
//    AAAA_FFFTeamCardViewController_BBBBOption *option = [[AAAA_FFFTeamCardViewController_BBBBOption alloc] init];
//    option.isTop = [NIMSDK.sharedSDK.chatExtendManager stickTopInfoForSession:self.session] != nil;
//
//    if (self.session.sessionType == NIMSessionTypeTeam) {
...
//            vc = [[AAAA_FFFAdvancedTeamCardViewController_BBBB alloc] initWithTeam:team
...
- (void)doMergerForwardToSession:(NIMSession *)session {
    __weak typeof(self) weakSelf = self;
    AAAA_USERMergeForward_BBBBTask *task = [_mergeForwardSession forwardTaskWithMessages:_selectedMessages process:nil completion:^(NSError * _Nonnull error, NIMMessage * _Nonnull message) {
        if (error) {
...
- (void)switchUIWithSessionState:(AAAA_UserSessionState_BBBB)state {
    switch (state) {
        case AAAA_UserSessionState_BBBBSelect:
        {
            [self setupSelectedNav];
            [self setSessionState:AAAA_UserSessionState_BBBBSelect];
            [self.view addSubview:self.mulSelectedSureBar];
            break;
        }
        case AAAA_UserSessionState_BBBBNormal:
        default:
        {
            [self.mulSelectedSureBar removeFromSuperview];
            [self setSessionState:AAAA_UserSessionState_BBBBNormal];
//            [self setupNormalNav];
...
- (void)cancelSelected:(id)sender {
    [self switchUIWithSessionState:AAAA_UserSessionState_BBBBNormal];
}
...
- (void)confirmSelected:(id)sender {
    ...
    [self selectForwardSessionCompletion:^(NIMSession *targetSession) {
        ...
        //返回正常页面
        [weakSelf switchUIWithSessionState:AAAA_UserSessionState_BBBBNormal];
    }];
}
...
- (void)revokeMessage:(id)sender
{
    ...
    [[NIMSDK sharedSDK].chatManager revokeMessage:message
                                      apnsContent:@"撤回一条消息"
                                      apnsPayload:payload
                                  shouldBeCounted:![[AAAA_USERBundleSetting_BBBB sharedConfig] isIgnoreRevokeMessageCount]
                                         completion:^(NSError * _Nullable error)
    {
        if (error) {
            if (error.code == NIMRemoteErrorCodeDomainExpireOld) {
            ...
            AAAA_FFFMessageModel_BBBB *model = [weakSelf uiDeleteMessage:message];
            NIMMessage *tip = [AAAA_USERSessionMsgConverter_BBBB msgWithTip:[AAAA_USERSessionUtil_BBBB tipOnMessageRevoked:nil]];
            tip.timestamp = model.messageTime;
...
- (AAAA_USERMulSelectFunctionBar_BBBB *)mulSelectedSureBar {
    if (!_mulSelectedSureBar) {
        _mulSelectedSureBar = [[AAAA_USERMulSelectFunctionBar_BBBB alloc] initWithFrame:self.sessionInputView.frame];
...
- (AAAA_ZMONTranslateView_BBBB *)translateView
{
    if(!_translateView){
        _translateView = [[AAAA_ZMONTranslateView_BBBB alloc]initWithFrame:CGRectMake(0, 0, SCREEN_WIDTH, SCREEN_HEIGHT)];
...
- (void)didTouchSubmitContentButton:(NSString *)reason
{
    ...
    if(isMyFriend){
        ...
        NSMutableDictionary *dict = @{}.mutableCopy;
        dict[@"contact"] = reason;
        [AAAA_HttpManager_BBBB postWithUrl:Server_other_feedback params:dict isShow:NO success:^(id responseObject) {
//            [AAAA_SVProgressHUD_BBBB showMessage:LangKey(@"report_sucessed")];
        } failed:^(id responseObject, NSError *error) {
        }];
    
}

- (AAAA_ZMONReport_BBBBNextView *)reprotNextView
{
    if(!_reprotNextView){
        _reprotNextView  = [[AAAA_ZMONReport_BBBBNextView alloc]initWithFrame:CGRectMake(0, 0, SCREEN_WIDTH, SCREEN_HEIGHT)];
...
//    self.reprotDeleteView.userID = self.userId
    ...
    self.reprotDeleteView.speiceBackBlock = ^(NSString *Name){
        ...
        [[NIMSDK sharedSDK].userManager deleteFriend:self.userId
                                         removeAlias:[[AAAA_USERBundleSetting_BBBB sharedConfig] autoRemoveAlias]
                                          completion:^(NSError *error) {
...
- (AAAA_ZMONReport_BBBBBlackView *)reprotBlackView
{
    if(!_reprotBlackView){
        _reprotBlackView  = [[AAAA_ZMONReport_BBBBBlackView alloc]initWithFrame:CGRectMake(0, 0, SCREEN_WIDTH, SCREEN_HEIGHT)];
    }
    return _reprotBlackView;

}

- (AAAA_ZMONReport_BBBBDeleteView *)reprotDeleteView
{
    if(!_reprotDeleteView){
        _reprotDeleteView  = [[AAAA_ZMONReport_BBBBDeleteView alloc]initWithFrame:CGRectMake(0, 0, SCREEN_WIDTH, SCREEN_HEIGHT)];
    }
    return _reprotDeleteView;
}

- (AAAA_ZMONReport_BBBBView *)reprotHisNextView
{
    if(!_reprotHisNextView){
        _reprotHisNextView  = [[AAAA_ZMONReport_BBBBView alloc]initWithFrame:CGRectMake(0, 0, SCREEN_WIDTH, SCREEN_HEIGHT)];
