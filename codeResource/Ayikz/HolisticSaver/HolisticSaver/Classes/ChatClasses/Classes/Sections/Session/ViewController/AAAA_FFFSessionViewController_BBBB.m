//
//  AAAA_FFFSessionViewController_BBBB.m
// AAAA_MyUserKit_BBBB
//
//  Created by NetEase.
...
@interface AAAA_FFFSessionViewController_BBBB ()<NIMMediaManagerDelegate,AAAA_NIMInputDelegate_BBBB,AAAA_FFFTeamCardViewControllerDelegate_BBBB,NIMConversationManagerDelegate,AAAA_CustomUISeletePhotosDelegate_BBBB,UIGestureRecognizerDelegate>
...
@property (nonatomic,strong)    AAAA_FFFSessionConfigurator_BBBB *configurator;

@property (nonatomic,strong)    UITapGestureRecognizer *tableViewTapGesture;

@property (nonatomic,assign) NSInteger sessionUnreadCount;
@property (nonatomic,strong) AAAA_FFFBadgeView_BBBB *badgeView;
...
@property (nonatomic, strong) AAAA_FFFInputAudioView_BBBB *audioContent;

@property (nonatomic, strong) UIView *photopicview;
@property (nonatomic, strong) AAAA_PhotoContainerView_BBBB *customAlbumView;


@end

@implementation AAAA_FFFSessionViewController_BBBB
...
- (void)setupNav
{
    ...
    self.sessionUnreadCount = [NIMSDK sharedSDK].conversationManager.allUnreadCount;
     _badgeView = [AAAA_FFFBadgeView_BBBB viewWithBadgeTip:@""];
...
- (void)setupInputView
{
    ...
    {
        self.sessionInputView = [[AAAA_FFFInputView_BBBB alloc] initWithFrame:CGRectMake(0, 0, self.view.device_width,0) config:self.sessionConfig];
...
{
    _configurator = [[AAAA_FFFSessionConfigurator_BBBB alloc] init];
...
#pragma mark - AAAA_FFFSessionConfigurator_BBBBDelegate
...
- (NSString *)sessionTitle
{
    ...
    switch (type) {
        ...
        case NIMSessionTypeP2P:{
            title = [AAAA_FFFKitUtil_BBBB showNick:self.session.sessionId inSession:self.session];
        }
            break;
        case NIMSessionTypeSuperTeam: {
            NIMTeam *team = [[[NIMSDK sharedSDK] superTeamManager] teamById:self.session.sessionId];
...
- (void)setSessionState:(AAAA_UserSessionState_BBBB)state {
...
{
    id<AAAA_NIMSessionInteractor_BBBB> interactor = self.interactor;
...
- (void)onRecvMessages:(NSArray *)messages
{
    if ([self shouldAddListenerForNewMsg])
    {
        ...
        if (![topvc isKindOfClass:NSClassFromString(@"AAAA_USERSessionViewController_BBBB")] && ![topvc isKindOfClass:[AAAA_FFFSessionViewController_BBBB class]]) {
            return;
        }
...
- (void)fetchMessageAttachment:(NIMMessage *)message didCompleteWithError:(NSError *)error
{
    if ([message.session isEqual:_session])
    {
        AAAA_FFFMessageModel_BBBB *model = [self.interactor findMessageModel:message];
...
- (void)recordAudio:(NSString *)filePath didCompletedWithError:(NSError *)error {
    if(!error) {
        if ([self recordFileCanBeSend:filePath]) {
            [self sendMessage:[AAAA_FFFMessageMaker_BBBB msgWithAudio:filePath]];
        }else{
...
#pragma mark - AAAA_NIMInputDelegate_BBBB
...
#pragma mark - AAAA_NIMInputActionDelegate_BBBB
...
//    [self praiseViewdidSelectIndex:index];
    ...
    __block NIMQuickComment *newComment = [AAAA_NIMCommentMaker_BBBB commentWithType:index content:@"" ext:@"扩展"];
...
- (void)onSendText:(NSString *)text atUsers:(NSArray *)atUsers
{
    ...
    NIMMessage *message = [AAAA_FFFMessageMaker_BBBB msgWithText:text];
    if (atUsers.count)
    {
        ...
        NSString *me = [[AAAA_MyUserKit_BBBB sharedKit].provider infoByUser:[NIMSDK sharedSDK].loginManager.currentAccount option:option].showName;
...
- (void)onSendText:(NSString *)text atUsers:(NSArray *)atUsers replymessage:(NIMMessage *)replymessage
{
    ...
    AAAA_FFFKitInfo_BBBB *info = [[AAAA_MyUserKit_BBBB sharedKit] infoByUser:replymessage.from option:option];
...
- (void)onSelectEmoticon:(AAAA_NIMInputEmoticon_BBBB *)emoticon
{
    ...
    __block NIMQuickComment *newComment = [AAAA_NIMCommentMaker_BBBB commentWithType:number content:emoticon.tag ext:@"扩展"];
    
    __weak typeof(self) weakSelf = self;
    [self hadCommentThisMessage:self.messageForMenu type:number
                      compltion:^(NSMapTable *result)
     {
        NIMQuickComment *oldComment = [AAAA_FFFKitQuickCommentUtil_BBBB myCommentFromComments:0 keys:@[@(number)] comments:result];
...
//
//    NIMMessage *message = [AAAA_FFFMessageMaker_BBBB msgWithImage:gif];
//    [self sendMessage:message];
//
    
    NIMMessage *message = [AAAA_FFFMessageMaker_BBBB msgWithImageData:imageData extension:@""];
...
//    [_sessionInputView endEditing:YES];
    ...
    NSTimeInterval duration = [AAAA_MyUserKit_BBBB sharedKit].config.recordMaxDuration;
...
#pragma mark - AAAA_FFFMessageCellProtocol_BBBBFFFMessageCell_BBBBDelegate_BBBB
...
- (void)copyText:(id)sender
{
    ...
    if (message.AAAA_Tyl_internalIdentifier_BBBB.length) {
        UIPasteboard *pasteboard = [UIPasteboard generalPasteboard];
        [pasteboard setString:message.AAAA_Tyl_internalIdentifier_BBBB];
    }
...
- (void)onTapMenuItemCopy:(AAAA_FFFMediaItem_BBBB *)item
{
    NIMMessage *message = [self messageForMenu];
    if (message.AAAA_Tyl_internalIdentifier_BBBB.length) {
        UIPasteboard *pasteboard = [UIPasteboard generalPasteboard];
...
- (AAAA_FFFMessageModel_BBBB *)uiDeleteMessage:(NIMMessage *)message{
    AAAA_FFFMessageModel_BBBB *model = [self.interactor deleteMessage:message];
...
- (void)onTapMediaItemPicture:(AAAA_FFFMediaItem_BBBB *)item
{
    [self.interactor mediaPicturePressed];
}

- (void)onTapMediaItemShoot:(AAAA_FFFMediaItem_BBBB *)item
{
    [self.interactor mediaShootPressed];
}

- (void)onTapMediaItemLocation:(AAAA_FFFMediaItem_BBBB *)item
{
...
{
//    AAAA_FFFBubbleMenuView_BBBB.shareMenuView.praiseView.hidden = YES;
//    AAAA_FFFBubbleMenuView_BBBB.shareMenuView.buttonsBgView.hidden = NO;
    [AAAA_FFFBubbleMenuView_BBBB.shareMenuView removeFromSuperview];
    [self.sessionInputView endEditing:YES];
...
- (void)setUpTitleView
{
    AAAA_FFFKitTitleView_BBBB *titleView = (AAAA_FFFKitTitleView_BBBB *)self.navigationItem.titleView;
    if (!titleView || ![titleView isKindOfClass:[AAAA_FFFKitTitleView_BBBB class]])
    {
        titleView = [[AAAA_FFFKitTitleView_BBBB alloc] initWithFrame:CGRectZero];
...
- (void)showMenuWithMessage:(NIMMessage *)message withView:(UIView *)view {
    ...
    if (btns.count > 0) {
        ...
        @weakify(self);
        [[AAAA_FFFBubbleMenuView_BBBB shareMenuView] showViewWithButtonModels:btns cursorStartRect:cursorStartRectToWindow selectionTextRectInWindow:tempRect selectBlock:^(AAAA_FFFMediaItem_BBBB *item) {
            @strongify(self);
...
- (void)praiseViewdidSelectIndex:(NSInteger)index
{
    ...
    [self hadCommentThisMessage:self.messageForMenu type:index
                      compltion:^(NSMapTable *result)
     {
        NIMQuickComment *oldComment = [AAAA_FFFKitQuickCommentUtil_BBBB myCommentFromComments:0 keys:@[@(index)] comments:result];
        BOOL contains = oldComment ? YES : NO;
        if (!contains)
        {
...
- (AAAA_PhotoContainerView_BBBB *)customAlbumView
{
    if(!_customAlbumView){
        _customAlbumView = [[AAAA_PhotoContainerView_BBBB alloc]initWithFrame:CGRectMake(0, 0, SCREEN_WIDTH, SCREEN_HEIGHT)];
...
//        btnAlbum.layer.cornerRadius = 20;
        ...
        [btnAlbum layoutButtonWithEdgeInsetsStyle:(AAAA_ImageTitleSpacing_BBBBEdgeInsetsStyleTop) imageTitleSpace:10];
...
//        btnShot.layer.cornerRadius = 20;
        ...
        [btnShot layoutButtonWithEdgeInsetsStyle:(AAAA_ImageTitleSpacing_BBBBEdgeInsetsStyleTop) imageTitleSpace:10];
        [_photopicview addSubview:btnShot];
    }
    return _photopicview;
}

- (AAAA_FFFInputAudioView_BBBB *)audioContent
{
    if(!_audioContent){
        _audioContent = [[AAAA_FFFInputAudioView_BBBB alloc]initWithFrame:CGRectMake(0, 0, SCREEN_WIDTH, SCREEN_HEIGHT)];
